CREATE OR REPLACE procedure ADPROD.USF_ADV_usfmail_update_p
/******************************************************************************
PURPOSE:
...
REVISIONS:
   ...
   1.0        5/6.2009   Yong Zhou      1. Created this procedure.
               02/28, 2011                       added custom table azbemil
               03,2, 2011                          only update the preferred ind for usf mail
               09,07,2012                          added the and e.GOREMAL_PREFERRED_IND = 'Y' to
                                                         only select the usf perferred eamils
...
cursor c_adv_email_rec is
  ...
  where  t.AZREMIL_PIDM = e.goremal_pidm
    ...
    and upper( t.AZREMIL_EMAIL_ADDRESS)  like upper( '%@usfca.edu')
...
BEGIN
       ...
       SELECT To_char(trunc(SYSDATE), 'YYYYMMDD') into v_dateStamp from dual;
        fHandler := UTL_FILE.FOPEN('ADV_MAIL_DIR', 'Preferred Ind Update Report for USF Email'||v_dateStamp||'.txt', 'W');
                       ...
                       --DBMS_OUTPUT.PUT_LINE('The next pidm is ' || v_adv_email_rec.spriden_pidm|| chr(13));
                        if v_goremal_rec.GOREMAL_PREFERRED_IND = 'Y' Then
                                 ...
                                 update GOREMAL set  GOREMAL_PREFERRED_IND = 'N',
                                                           GOREMAL_COMMENT = 'update preferred ind to N for usf email per Imods bounceback',
                                                           GOREMAL_ACTIVITY_DATE = sysdate,
                                                           GOREMAL_USER_ID = user
                                                           where current of c_goremal_rec;
                                  if sql%found Then
                                       report_row := 'Existing USF prefered Email ' || v_adv_email_rec.AZREMIL_EMAIL_ADDRESS || ' for person '
                                             || gb_common.f_get_id(v_adv_email_rec.AZREMIL_PIDM);
                                  ...
                                  If c_pers_email%found Then
                                       update GOREMAL set GOREMAL_PREFERRED_IND = 'Y',
                                           GOREMAL_COMMENT = 'update preferred ind for PERS mail per Imods bounceback',
                                           GOREMAL_ACTIVITY_DATE = sysdate,
                                           GOREMAL_USER_ID ='Advancement'
                                           where current of c_pers_email;
                                     if sql%found Then
                                        report_row := 'Update Preferred Email to PERS ' || v_adv_email_rec.AZREMIL_EMAIL_ADDRESS || ' for person ' || gb_common.f_get_id(v_adv_email_rec.AZREMIL_PIDM);
                                        UTL_FILE.PUT_LINE(fHandler, report_row || CHR(13));
                                     end if;
                                  elsif c_bus_email%found Then
                                          update GOREMAL set GOREMAL_PREFERRED_IND = 'Y',
                                           GOREMAL_COMMENT = 'update preferred ind for BUS mail per Imods bounceback',
                                           GOREMAL_ACTIVITY_DATE = sysdate,
                                           GOREMAL_USER_ID ='Advancement'
                                            where current of c_bus_email;
                                     if sql%found Then
                                        report_row := 'Update Preferred Email to BUS ' || v_adv_email_rec.AZREMIL_EMAIL_ADDRESS || ' for person ' || gb_common.f_get_id(v_adv_email_rec.AZREMIL_PIDM);
                                        UTL_FILE.PUT_LINE(fHandler, report_row || CHR(13));
                                     ENd if;
                                  ENd if;
                                  close c_pers_email;
                                  close c_bus_email;
                                   ...
                                   report_row := 'No Action for  ' || v_adv_email_rec.AZREMIL_EMAIL_ADDRESS || ' for person ' || gb_common.f_get_id(v_adv_email_rec.AZREMIL_PIDM);
                                   UTL_FILE.PUT_LINE(fHandler, report_row || CHR(13));
